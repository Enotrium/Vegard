"""AIP Bridge - Clean Syndar→AIP data pipeline

POSTs structured SoilPrediction payloads to AIP's `/api/syndar/ingest`.
Translates Syndar entity model to AIP farm pipeline schema.
Zero circular coupling - AIP never imports Syndar.
"""
...
class AIPBridge:
    """Bridge from Syndar to AIP farm pipeline"""
    ...
    def _convert_entity_to_aip(self, entity: EntityState) -> AIPSoilPayload:
        """Convert Syndar entity to AIP payload"""
        soil = entity.soil
